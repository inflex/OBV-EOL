...
char help[]=" [-h] [-l] [-c <config file>] [-i <intput file>]\n\
	-h : This help\n\
...
int main(int argc, char **argv)
{
	...
	SDL_Window *window = SDL_CreateWindow("OpenFlex Board Viewer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, g.width, g.height, SDL_WINDOW_OPENGL|SDL_WINDOW_RESIZABLE);
...
//		if (SDL_GetWindowFlags(window) & (SDL_WINDOW_MINIMIZED|SDL_WINDOW_HIDDEN)) { usleep(50000); continue; } // stops OVB/SDL consuming masses of CPU when it should be idling.
		if (!(sleepout--)) { usleep(50000); sleepout = 0; continue; } // puts OBV to sleep if nothing is happening.
