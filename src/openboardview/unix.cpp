...
const std::string get_font_path(const std::string &name) {
	...
	if (font) {
		...
		if (FcPatternGetString(font, FC_FAMILY, 0, &fcfamily) == FcResultMatch
			&& ( name.empty() || !FcStrCmpIgnoreCase(fcname, fcfamily) ) // Empty name means searching for default font, otherwise make sure the returned font is exactly what we searched for
